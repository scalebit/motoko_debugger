...
use crate::{
    ...
    engine::{
        code_map::CodeMap,
        executor_debugger::stack::{CallFrame, FrameRegisters, ValueStack},
...
pub fn execute_instrs<'engine, T>(
    ...
    Executor_debugger::new(stack, code_map, cache).execute(store)
}
...
pub fn execute_instrs_dbg<'engine, T>(
    ...
    let instance = stack.calls.instance_expect();
    let cache = CachedInstance::new(&mut store.inner, instance);
    Executor_debugger::new(stack, code_map, cache).execute_dbg(store)
}
...
#[derive(Debug)]
pub struct Executor_debugger<'engine> {
...
impl<'engine> Executor_debugger<'engine> {
    /// Creates a new [`Executor_debugger`] for executing a Wasmi function frame.
    #[inline(always)]
    pub fn new(
        stack: &'engine mut Stack,
        code_map: &'engine CodeMap,
    ...
    fn execute_dbg<T>(mut self, store: &mut Store<T>) -> Result<Signal, Error> {
        ...
        loop {
            match *self.ip.get() {
                ...
                Instr::ReturnNezReg { condition, value } => {
                    forward_return_dbg!(self.execute_return_nez_reg(
                        &mut store.inner,
                        condition,
                        value
                    ))
            ...
            fn $name(&self, index: $index_ty) -> $id_ty {
                unsafe { self.cache.$name(index) }
                    .unwrap_or_else(|| {
                        ...
                        // Safety: within the Wasmi executor_debugger it is assumed that store entity
...
impl Executor_debugger<'_> {
    ...
    fn get_register(&self, register: Reg) -> UntypedVal {
        // Safety: - It is the responsibility of the `Executor_debugger`
    ...
    /// Initializes the [`Executor_debugger`] state for the [`CallFrame`].
    ///
    /// # Note
    ///
    /// The initialization of the [`Executor_debugger`] allows for efficient execution.
...
/// Extension method for [`UntypedVal`] required by the [`Executor_debugger`].
