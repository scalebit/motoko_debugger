...
use wasi_common::{snapshots::preview_0::wasi_unstable::WasiUnstable, Error};
...
pub fn add_wasi_snapshot_preview0_to_linker<T, U>(
    linker: &mut Linker<T>,
    wasi_ctx: impl Fn(&mut T) -> &mut U + Send + Sync + Copy + 'static,
) -> Result<(), Error>
where
...
where
    U: WasiUnstable,
{
    <LinkerBuilder<Constructing, T> as AddWasiPreview0<T>>::add_wasi(linker, wasi_ctx)
}
...
macro_rules! add_funcs_to_linker {
    (
        ...
        impl<T> AddWasiPreview0<T> for $linker {
            fn add_wasi<U>(
                &mut self,
                wasi_ctx: impl Fn(&mut T) -> &mut U + Send + Sync + Copy + 'static,
            ) -> Result<(), Error>
                    ...
                    self.func_wrap(
                        ...
                        move |mut caller: Caller<'_, T>, $($arg : $typ,)*| -> Result<$ret, wasmi::Error> {
                            let result = async {
                                ...
                                match wasi_common::snapshots::preview_0::wasi_unstable::$fname(ctx, &mut memory, $($arg,)*).await {
                                    Ok(r) => Ok(<$ret>::from(r)),
